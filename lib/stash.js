...
Stash.prototype.substitutePath = function(path) {
  ...
  if(! _.isString(path)) return Q(path);
  var that = this;

  var halves = path.split("?");
  var pathParts = halves[0].split("/");
  var delimitedParts = _.map(pathParts, function(part) {
    return part.split(",");
  });
  var subbingPath = that.substitute(delimitedParts)
  .then( function(subbedParts) {
    return _.map(subbedParts, function(parts) {
      return parts.join(",");
    }).join("/");
  });
