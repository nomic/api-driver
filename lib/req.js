...
var reqAPI = {
  ...
  headers: function(headers) {
    return this.merge({reqOpts: {headers: headers}});
  },
  ...
  expect: function req$expect() {
    ...
    var expectation = function(ctx, actual) {
      expected = ctx.substitute(expected);
  ...
  defaultExpect: function(fn) {
    return this.merge({defaultExpectFn: fn});
  },
...
function execReq(clauses, ctx) {
  var reqOpts = _.extend(
    ctx.substitute( _.omit(clauses.reqOpts, 'relativeUrl') ),
    { relativeUrl: ctx.substituteRoute(clauses.reqOpts.relativeUrl) },
    { jar: ctx.jarForCurrentActor() }
  );
